                ...
                <Grid item xs={12} sm={6} md={4} key={content.id}>
                  ...
                  >
                    ...
                    <Box display="flex" justifyContent="space-between" mb={1}>
                      ...
                      <Typography sx={{ ...pixelText, fontSize: '8px', fontWeight: 'bold' }}>{content.contentType}</Typography>
                    </Box>
                    <Box display="flex" justifyContent="space-between" mb={1}>
                      <Typography sx={{ ...pixelText, fontSize: '8px' }}>Published:</Typography>
                      <Typography sx={{ ...pixelText, fontSize: '8px' }}>{new Date(content.createdAt).toLocaleDateString()}</Typography>
                    </Box>
                    <Button fullWidth variant="outlined" size="small" onClick={e => { e.stopPropagation(); handleContentSelect(content); }} sx={{ ...pixelButton, fontSize: '7px', mt: 1, borderColor: '#5F4B8B', color: '#5F4B8B', '&:hover': { backgroundColor: 'rgba(95, 75, 139, 0.1)' } }}>VIEW SESSIONS</Button>
                    <Button fullWidth variant="contained" size="small" startIcon={<Download />} onClick={e => { e.stopPropagation(); handleExportReports(content); }} sx={{ ...pixelButton, fontSize: '7px', mt: 1, backgroundColor: '#6c63ff', '&:hover': { backgroundColor: '#5a52e0' } }}>EXPORT REPORTS</Button>
                  </Paper>
                </Grid>
              ))}
            </Grid>
              ...
              {completedSessions.map(session => (
                <Grid item xs={12} sm={6} md={4} key={session.id}>
                  <Paper elevation={selectedSession === session.id ? 3 : 1} sx={{ p: 2, cursor: 'pointer', border: selectedSession === session.id ? '2px solid #5F4B8B' : '1px solid rgba(0,0,0,0.12)', transition: 'all 0.2s ease', position: 'relative', '&:hover': { transform: 'translateY(-2px)', boxShadow: '0 4px 10px rgba(95, 75, 139, 0.15)' } }} onClick={() => fetchStudentReports(session.id)}>
                    <Typography sx={{ ...pixelHeading, fontSize: '10px', color: '#5F4B8B' }}>Session #{session.id}</Typography>
                    <Typography sx={{ ...pixelText, fontSize: '8px' }}>Date: {new Date(session.createdAt || session.startedAt).toLocaleDateString()}</Typography>
                    <Typography sx={{ ...pixelText, fontSize: '8px' }}>Players: {session.playerCount || session.players?.length || 0}</Typography>
                    <Typography sx={{ ...pixelText, fontSize: '8px' }}>Status: {session.status || 'Completed'}</Typography>
                    ...
                    <Grid item xs={12} sm={6} md={4} key={student.userId}>
                      <Paper elevation={1} sx={{ p: 2, cursor: 'pointer', borderRadius: '8px', transition: 'all 0.2s ease', '&:hover': { transform: 'translateY(-2px)', boxShadow: '0 4px 10px rgba(95, 75, 139, 0.15)' } }} onClick={() => navigate(`/student-report/${selectedSession}/${student.userId}`)}>
                        <Box display="flex" alignItems="center" mb={1}>
                          <Avatar sx={{ bgcolor: '#6c63ff', width: 30, height: 30, mr: 1, fontSize: '12px' }}>{student.name.split(' ').map(n => n[0]).join('')}</Avatar>
                          <Typography sx={{ ...pixelText, fontWeight: 'bold' }}>{student.name}</Typography>
                        </Box>
                        <Box display="flex" justifyContent="space-between" mb={1}>
                          <Typography sx={{ ...pixelText, fontSize: '8px' }}>Role:</Typography>
                          <Typography sx={{ ...pixelText, fontSize: '8px', color: '#5F4B8B' }}>{student.role || 'N/A'}</Typography>
                        </Box>
                        <Box display="flex" justifyContent="space-between" mb={1}>
                          <Typography sx={{ ...pixelText, fontSize: '8px' }}>Score:</Typography>
                          <Typography sx={{ ...pixelText, fontSize: '8px', fontWeight: 'bold' }}>{student.totalScore}</Typography>
                        </Box>
                        <Box display="flex" justifyContent="space-between">
                          <Typography sx={{ ...pixelText, fontSize: '8px' }}>Feedback:</Typography>
                          <Chip size="small" label={student.hasFeedback ? 'PROVIDED' : 'NEEDED'} color={student.hasFeedback ? 'success' : 'warning'} sx={{ height: '16px', '& .MuiChip-label': { ...pixelText, fontSize: '6px', px: 1 } }} />
                        </Box>
                      </Paper>
                    </Grid>
                  ))}
